...
> exported by the runner's review environment (`$REVIEW_DATA`). If it is unset you are on a machine
> without one: pick a scratch directory on a real disk, say which in the closing summary, and do not
> guess. **Never `/tmp`** — it is a tmpfs on many machines, and filling it takes the desktop down
> with it. Remove the dir when the review is done.
>
...
Measured: `curl https://api.openai.com/v1/models` inside gives
...
- **RSYNC mode** — the literal word `rsync`. A **completely separate** review target from all ArduPilot
  ...
  its own single-page report at `$REVIEW_PUBLIC_URL/RsyncReviews/index.html` and its own manifest,
...
| published "latest" | `$REVIEW_PUBLIC_URL/DevCallReviews/<LABEL>/devcall_pr_reviews.html` | `$REVIEW_PUBLIC_URL/UserReviews/<USERNAME>.html` | `$REVIEW_PUBLIC_URL/DevCallReviews/followups/<DATE_TIME>/devcall_pr_reviews.html` (one report per run), **plus** a refresh of every per-label report containing a re-reviewed PR | `$REVIEW_PUBLIC_URL/RsyncReviews/index.html` (a single living page — destination is the filename `index.html`, not a directory) |
| dated archive | `$REVIEW_PUBLIC_URL/DevCallReviews/<DATE>/devcall_pr_reviews.html` | none | n/a — the run's own report **is** the dated one | none — the single page is simply kept current |
...
`$REVIEW_PUBLIC_URL/DevCallReviews/followups/<DATE_TIME>/devcall_pr_reviews.html` (one directory
...
1. Find the PRs to review. **Capture each PR's current head commit** (`headRefOid`); the short hash is its first 10 characters.
   ...
   **FOLLOWUP mode** — the set is derived from what has already been reviewed and commented on, not from
   ...
   1. **List the published reports.** `curl -fsS $REVIEW_PUBLIC_URL/DevCallReviews/` returns a
      ...
      ```bash
      curl -fsS $REVIEW_PUBLIC_URL/DevCallReviews/ \
...
2. **Fast incremental skip — decide what actually needs reviewing.**
   ...
   **In FOLLOWUP mode step 1 has already done this**, and by a stricter test (posted-comment head, plus
   ...
   - LABEL mode: `curl -fsS $REVIEW_PUBLIC_URL/DevCallReviews/$ARGUMENTS/devcall_pr_reviews.html`
     AUTHOR mode: `curl -fsS $REVIEW_PUBLIC_URL/UserReviews/<USERNAME>.html`
     RSYNC mode: `curl -fsS $REVIEW_PUBLIC_URL/RsyncReviews/index.html`
...
8. **Post review comments to the PRs — LABEL mode (DevCallEU/DevCallTopic/AIReview) and RSYNC mode.** This step runs automatically when the mode is LABEL and the label is exactly **`DevCallEU`**, **`DevCallTopic`**, or **`AIReview`**, **and in RSYNC mode** (`RsyncProject/rsync`, `AIReview` label — tridge's own project, opted in). The **`AIReview` label on an ArduPilot PR is the same opted-in signal as on the rsync repo** — someone applied it to request an auto-posted AI review — so it auto-posts exactly like the dev-call labels (same scope, AI-generated marker, and edit-vs-repost mechanics below); it simply has no associated dev call, so its dated archive uses today's date (step 9), not a call date. **Exception: PRs in `mavlink/mavlink` are never auto-commented** (this covers an `AIReview`-labelled upstream-mavlink PR too). That is a third-party upstream project rather than an ArduPilot one — a posted review there goes to maintainers who have not opted into this process and lands under the running user's name on someone else's repo. Review and include those PRs in the report exactly as normal, then **hold** the comment: record it in the report as `comment held — upstream repo, needs approval`, name it in the closing summary with the exact `gh pr comment` that would post it, and carry on. **Do not wait for a yes** — under a schedule nobody is there to give one, and blocking here throws away a completed sweep for a comment that can just as well go out a day later (**Unattended runs** rule 3). In `--interactive` mode you may ask instead. Everything else about the mechanics is unchanged. For every other label, do **not** post any comments unless the user explicitly asks you to in their message. (When they do ask for another label, follow the same mechanics below.)
   ...
   **RSYNC mode posts exactly like the dev-call labels, with two specifics.** (a) Post only **after both the
   ...
   `$REVIEW_PUBLIC_URL/RsyncReviews/index.html#pr<number>`. The comment scope (comment on every
   ...
   **AUTHOR mode does not post, ever, unless explicitly asked in the user's message.** The default is a
   ...
   - **Link the comment to the report it came from**, on its own line near the top, so the author can see
     ...
     - LABEL mode → the **DATED archive**, never the per-label "latest":
       `$REVIEW_PUBLIC_URL/DevCallReviews/<DATE>/devcall_pr_reviews.html#pr<key>`, where `<DATE>` is
       ...
       **Never link `DevCallReviews/<LABEL>/devcall_pr_reviews.html` from a comment.** That path is
       ...
       FOLLOWUP/RSYNC.
     - AUTHOR mode → `$REVIEW_PUBLIC_URL/UserReviews/<USERNAME>.html` (only relevant if the user
     ...
     - FOLLOWUP mode → `$REVIEW_PUBLIC_URL/DevCallReviews/followups/<DATE_TIME>/devcall_pr_reviews.html`
     ...
     - RSYNC mode → `$REVIEW_PUBLIC_URL/RsyncReviews/index.html#pr<number>` — the single living page
...
9. **Publish the report to the web.** Once the report is finalised (including the Codex revisions and any comment posting), publish it to BOTH the per-label "latest" directory (which the next re-run reads) and a dated archive directory. This is automatic — always do it at the end of the run.
   ...
   **Publishing is parameterised**, because every site publishes somewhere different. Use
   `$REVIEW_PUBLISH` as the destination base, `$RSYNC_AUTH` for its credentials, and
   `$REVIEW_PUBLIC_URL` for the public address those files appear at; all three are exported by the
   runner's review environment (`runner/bin/review-env.sh`, set in `runner/etc/local.conf`).

   | transport | `$REVIEW_PUBLISH` | `$RSYNC_AUTH` |
   |---|---|---|
   | rsync daemon | `rsync://<user>@<host>` | `--password-file=<path>` |
   | ssh | `<host>:<path>` | *(empty)* |

   The path *after* the base is identical either way, so only those variables change, and `--mkpath`
   still creates the per-label / per-date subdirectory. **If `$REVIEW_PUBLISH` is unset** you are on a
   machine without a review environment: skip publishing, keep the report local, and say so in the
   closing summary. Never invent a destination.

   `$REVIEW_PUBLIC_URL` may resolve to a LAN address on the runner itself while being a public name
   elsewhere; that is fine and expected. Use it exactly as exported — do not rewrite it to a local
   name, because the URLs are also what humans click in the PR comments.
   ...
   **FOLLOWUP mode writes its own dated report under `followups/`, and additionally refreshes the label
   ...
   - LABEL mode serves at `$REVIEW_PUBLIC_URL/DevCallReviews/<LABEL>/devcall_pr_reviews.html`, with the dated snapshot at `$REVIEW_PUBLIC_URL/DevCallReviews/<DATE>/devcall_pr_reviews.html` — where `<DATE>` is the call date computed above, so for the dev-call labels that URL is usually in the future when you print it. Print it anyway: it is the link to hand out ahead of the meeting.
   - AUTHOR mode serves at `$REVIEW_PUBLIC_URL/UserReviews/<USERNAME>.html`. Because it is one page per user rather than a directory, publishing **replaces** the previous run's page — that is intended, since the 7-day window means an archive of author sweeps would mostly be duplicates. Print the URL when done, as for LABEL mode.
   - RSYNC mode serves at `$REVIEW_PUBLIC_URL/RsyncReviews/index.html`. It is one living page listing every currently-open `AIReview` PR; publishing **replaces** it each run (with reused sections carried over verbatim), so it always reflects the current set — a PR that closed/merged or lost the label simply drops off. There is no dated archive. Print the URL when done.
   - `--mkpath` creates the destination directory if it doesn't exist (the per-label and per-date directories under the publish base). If a host's rsync predates 3.2.3 and lacks `--mkpath`, create the directory on the host first (`ssh <host> mkdir -p <path>`, or an equivalent for an rsync daemon), then rsync without `--mkpath`.
...
`$REVIEW_PUBLIC_URL/DevCallReviews/<LABEL>/devcall_pr_reviews.html` (LABEL mode),
`$REVIEW_PUBLIC_URL/UserReviews/<USERNAME>.html` (AUTHOR mode), or
`$REVIEW_PUBLIC_URL/RsyncReviews/index.html` (RSYNC mode).
